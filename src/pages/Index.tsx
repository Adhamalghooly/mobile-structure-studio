...
const Index = () => {
  ...
  const runAnalysis = () => {
    ...
    if (selectedEngine === 'fem_coupled' && !ignoreSlab) {
      ...
      try {
        ...
        if (detectedConnections.length > 0) {
          ...
          if (hasBobFrame) {
            const results3D = getFrameResults3D(frames, beamsWithLoads, columns, mat, frameEndReleases, detectedConnections, slabs, slabProps, false);
    ...
    const bMap = new Map(beamsWithLoads.map(b => [b.id, b]));
    if (removedColumnIds.length > 0 && detectedConnections.length > 0) {
      const beamHinges3D = build2DHingeMap();
      const result = analyzeWithBeamOnBeam(frames, bMap, columns, mat, removedColumnIds, detectedConnections, 10, 0.01, beamHinges3D);
      dispatch({ type: 'SET_FRAME_RESULTS', results: result.frameResults });
      dispatch({ type: 'SET_BOB_CONNECTIONS', connections: result.connections });
      if (!result.converged) {
  ...
  const frameResults2D = useMemo(() => {
    ...
    return frames.map(f => analyzeFrame(f, bMap, columns, mat, undefined, undefined, beamHinges2D));
  }, [analyzed, frames, beamsWithLoads, columns, mat, getBeamReleaseState]);
  ...
  const frameResults3DRaw = useMemo(() => {
    ...
    try {
      return getFrameResults3D(frames, beamsWithLoads, columns, mat, frameEndReleases, detectedConnections, slabs, slabProps, true);
    } catch {
      return [] as FrameResult[];
    }
  }, [analyzed, frames, beamsWithLoads, columns, mat, frameEndReleases, detectedConnections, slabs, slabProps]);
  ...
  const colLoads3D = useMemo(() => {
    ...
    try {
      return getColumnLoads3D(frames, beamsWithLoads, columns, mat, frameEndReleases, detectedConnections, slabs, slabProps, true);
    } catch {
      // Fallback to 2D if 3D fails
      return colLoadsBiaxial;
    }
  }, [analyzed, frames, beamsWithLoads, columns, mat, colLoadsBiaxial, slabs, slabProps]);
